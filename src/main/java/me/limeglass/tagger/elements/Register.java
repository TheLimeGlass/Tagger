package me.limeglass.tagger.elements;
...
import me.limeglass.tagger.Metrics;
import me.limeglass.tagger.Tagger;
import me.limeglass.tagger.Syntax;
import me.limeglass.tagger.utils.EnumClassInfo;
import me.limeglass.tagger.utils.TypeClassInfo;
import me.limeglass.tagger.utils.annotations.*;
...
public class Register {
	...
	public Register() {
		try {
			...
			File file = (File) method.invoke(Tagger.getInstance());
			...
			for (Enumeration<JarEntry> jarEntry = jar.entries(); jarEntry.hasMoreElements();) {
				...
				if (name.startsWith(Tagger.getPackageName()) && name.endsWith(".class")) {
		...
		run : for (Class clazz : classes) {
			if (!clazz.isAnnotationPresent(Disabled.class)) {
				...
				if (clazz.isAnnotationPresent(Patterns.class)) {
					...
					if (syntax == null) Tagger.debugMessage("&cThere was an issue registering the syntax for " + clazz.getName());
				} else {
					continue run;
				}
				...
				if (syntax != null) {
					if (Effect.class.isAssignableFrom(clazz)) {
						...
						Tagger.debugMessage("&5Registered Effect " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
					} else if (Condition.class.isAssignableFrom(clazz)) {
						Skript.registerCondition(clazz, syntax);
						Tagger.debugMessage("&5Registered Condition " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						...
						try {
							...
							Tagger.debugMessage("&5Registered Expression " + type.toString() + " " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						} catch (IllegalAccessException | IllegalArgumentException | InstantiationException e) {
							Tagger.consoleMessage("&cFailed to register expression " + clazz.getCanonicalName());
							e.printStackTrace();
						}
					}
				}
	...
	public static void metrics(Metrics metrics) {
		...
		Tagger.debugMessage("Metrics registered!");
	}
}