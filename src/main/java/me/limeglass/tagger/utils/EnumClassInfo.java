package me.limeglass.tagger.utils;
...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Tagger.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Tagger.getSyntaxData().set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Tagger.getInstance().getConfig().getString("syntaxDebug"))) Tagger.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Tagger.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Tagger.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Syntax.save();
	}
}