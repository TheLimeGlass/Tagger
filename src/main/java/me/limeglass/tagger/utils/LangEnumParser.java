...
import me.limeglass.tagger.lang.TaggerParser;

public class LangEnumParser<T extends Enum<T>> extends TaggerParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			Tagger.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			Tagger.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
		Syntax.save();
	}
