...
import me.limeglass.tagger.utils.annotations.DetermineSingle;
import me.limeglass.tagger.utils.annotations.Events;
import me.limeglass.tagger.utils.annotations.Multiple;
import me.limeglass.tagger.utils.annotations.Settable;
import me.limeglass.tagger.utils.annotations.Single;

public abstract class TaggerExpression<T> extends SimpleExpression<T> implements DataChecker {
	...
	public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parser) {
		if (getClass().isAnnotationPresent(Events.class)) {
			if (!ScriptLoader.isCurrentEvent(getClass().getAnnotation(Events.class).value())) {
				Tagger.debugMessage("The expression `" + getClass().getSimpleName() + "` can't be used in the event: " + ScriptLoader.getCurrentEventName());
				return false;
			}
		}
	...
	public String toString(Event event, boolean debug) {
		...
		if (event != null) Tagger.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
