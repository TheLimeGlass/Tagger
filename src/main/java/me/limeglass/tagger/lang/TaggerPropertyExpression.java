...
import me.limeglass.tagger.utils.annotations.Properties;
import me.limeglass.tagger.utils.annotations.Settable;

public abstract class TaggerPropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	public String toString(Event event, boolean debug) {
		...
		return Tagger.getNameplate() + getClass().getSimpleName() + ": the " + getPropertyName() + " (of|from) " + getExpr().toString(event, debug);
	}
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			Tagger.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
