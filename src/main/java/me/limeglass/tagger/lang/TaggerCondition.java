...
public abstract class TaggerCondition extends Condition implements DataChecker {
	...
	public String toString(Event event, boolean debug) {
		...
		if (event == null) {
			Tagger.debugMessage(getClass().getSimpleName() + " - " + modSyntax);
		} else {
			Arrays.asList(expressions.getExpressions()).stream().forEach(expression->values.add(expression.toString(event, debug)));
			Tagger.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
		}
		return Tagger.getNameplate() + getClass().getSimpleName() + "- Syntax: " + getSyntax();
	}
